<?php
    ...
    if (isset($_SESSION['student_id'])) {
        ...
        $q1_answer_query = "SELECT answer FROM Questions WHERE student_id='$id' AND question_number='$two'";
...
<section>
    <div class="container">
        <?php
            if (isset($_POST["treasureDepth"])) {
                $userSubmission = $_POST["treasureDepth"];
                if($q1_answer == $userSubmission){ // question was answered correctly
                    $two =2;
                    $finished_at_query = "SELECT question_finished_at FROM Questions WHERE student_id='$id' AND question_number='$two'";
                    // if (see_if_timer_has_been_started($conn, $finished_at_query) == 1){ //we need to insert a timer
                    //     $access_q1_at = date('Y-m-d H:i:s', time());
                    //     $update_time_query = "UPDATE Questions SET question_finished_at = '$access_q1_at' WHERE student_id='$id' AND question_number='$two'";
                    //     mysqli_query($conn, $update_time_query);

                    //     // now we need to update the duration
                    //     $started_at_result = mysqli_query($conn, "SELECT question_start_at FROM Questions WHERE student_id='$id' AND question_number='$two'");
                    //     $finished_at_result = mysqli_query($conn, "SELECT question_finish_at FROM Questions WHERE student_id='$id' AND question_number='$two'");
                    ...
                    //     $finish_val = $finish_row['question_finish_at'];
                    
                    //     $total_time = calculateTimeDuration($start_val, $finish_val);

                    //     echo $total_time;
                    
                    //     $update_total_time_query = "UPDATE Questions SET total_time_taken = '$total_time' WHERE student_id='$id' AND question_number='$two'";
                    //     mysqli_query($conn, $update_total_time_query);
                    // }
                    ...
                    if (see_if_timer_has_been_started($conn, $finished_at_query) == 1) { 
                        ...
                        if ($result_update) {
                            ...
                            $started_at_result = mysqli_query($conn, "SELECT question_start_at FROM Questions WHERE student_id='$id' AND question_number='$two'");
                            $finished_at_result = mysqli_query($conn, "SELECT question_finished_at FROM Questions WHERE student_id='$id' AND question_number='$two'");
                            ...
                            $current_amount_of_attempts_query = "SELECT attempts FROM Questions WHERE student_id='$id' and question_number='$two'";
                    ...
                    $update_db_query = "UPDATE Questions SET attempts=attempts+1 WHERE student_id='$id' AND question_number='$two'";
                    $update_correctly_answered_query = "UPDATE Questions SET has_been_correctly_answered=1 WHERE student_id='$id' AND question_number='$two'";
                ...
                elseif(isset($_POST["treasureDepth"]) && strlen($_POST["treasureDepth"]) > 20){
                    echo "<h5 class='main_body' style='padding-top:30px;'>Hmmm, your answer was longer than 20 characters, that's fishy. Your account is now being monitored - big brother is watching. Please go back and <a href='https://turing.plymouth.edu/~lmc1076/question2.php' style='color:yellow'>try again</a> with a real input.</h5>";
                    $update_db_query = "UPDATE Questions SET attempts=attempts+1 WHERE student_id='$id' AND question_number='$two'";
                    update_attempts($conn, $update_db_query);
                }
                else{
                    echo strlen($_POST["treasureDepth"]);
                    $update_db_query = "UPDATE Questions SET attempts=attempts+1 WHERE student_id='$id' AND question_number='$two'";
                    update_attempts($conn, $update_db_query);
                    echo "<h5 class='main_body' style='padding-top:30px;'>Your answer of $userSubmission was incorrect, please go back and <a href='https://turing.plymouth.edu/~lmc1076/question2.php' style='color:yellow'>try again.</a></h5>";
                }
            } else {
