<?php
    ...
    if (isset($_SESSION['student_id'])) {
        ...
        $q1_answer_query = "SELECT answer FROM Questions WHERE student_id='$id' AND question_number='$four'";
        ...
        <?php
            if (isset($_POST["treasureDepth_q3"])) {
                ...
                if($q1_answer == $userSubmission){ // question was answered correctly
                    $four = 4;
                    $finished_at_query = "SELECT question_finished_at FROM Questions WHERE student_id='$id' AND question_number='$four'";
                    ...
                    if (see_if_timer_has_been_started($conn, $finished_at_query) == 1) { 
                        ...
                        $update_time_query = "UPDATE Questions SET question_finished_at = '$access_q1_at' WHERE student_id='$id' AND question_number='$four'";
                        $result_update = mysqli_query($conn, $update_time_query);
                    
                        if ($result_update) {
                            // Fetch the start and finish timestamps
                            $started_at_result = mysqli_query($conn, "SELECT question_start_at FROM Questions WHERE student_id='$id' AND question_number='$four'");
                            $finished_at_result = mysqli_query($conn, "SELECT question_finished_at FROM Questions WHERE student_id='$id' AND question_number='$four'");
                            ...
                            $update_total_time_query = "UPDATE Questions SET total_time_taken = '$total_time' WHERE student_id='$id' AND question_number='$four'";
                            $result_total_time_update = mysqli_query($conn, $update_total_time_query);

                            // Fetch the current number of attempts from the database
                            $current_amount_of_attempts_query = "SELECT attempts FROM Questions WHERE student_id='$id' and question_number='$four'";
                    ...
                    $update_db_query = "UPDATE Questions SET attempts=attempts+1 WHERE student_id='$id' AND question_number='$four'";
                    $update_correctly_answered_query = "UPDATE Questions SET has_been_correctly_answered=1 WHERE student_id='$id' AND question_number='$four'";
                ...
                elseif(isset($_POST["treasureDepth_q3"]) && strlen($_POST["treasureDepth_q3"]) > 20){
                    echo "<h5 class='main_body' style='padding-top:30px;'>Hmmm, your answer was longer than 20 characters, that's fishy. Your account is now being monitored - big brother is watching. Please go back and <a href='https://turing.plymouth.edu/~lmc1076/question4.php' style='color:yellow'>try again</a> with a real input.</h5>";
                    $update_db_query = "UPDATE Questions SET attempts=attempts+1 WHERE student_id='$id' AND question_number='$four'";
                    update_attempts($conn, $update_db_query);
                }
                else{
                    $update_db_query = "UPDATE Questions SET attempts=attempts+1 WHERE student_id='$id' AND question_number='$four'";
                    update_attempts($conn, $update_db_query);
                    echo "<h5 class='main_body' style='padding-top:30px;'>Your answer of $userSubmission was incorrect, please go back and <a href='https://turing.plymouth.edu/~lmc1076/question4.php' style='color:yellow'>try again.</a></h5>";
                }
            } else {
